...
void solve(){
    ...
    function<void(ll, ll, ll)> dfs_down;
    function<ll(ll, ll)> dfs_up;

    dfs_up = [&](ll u, ll pv) -> ll{
    	ll mysum = 0;
    	for(auto p: mp[u]){
    		mysum += p.second;
    	}
    	...
    	ll ret = ans[u] - mp[u][pv];
    	if(pv != -1)
    		ans[u] = mp[u][pv];
    	return ret;
    };

    dfs_down = [&](ll u, ll pv, ll down){
    	ans[u] += down;
    	for(ll nxt: g[u]){
    		if(nxt != pv){
    			dfs_down(nxt, u, ans[u] - mp[u][nxt]);
    		}
    	}
    };
