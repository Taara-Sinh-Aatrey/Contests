...
void solve() {
    ...
    for(int i = 1; i < 10; i++) {
        for(int j = 1; j < 10; j++) {
            int ways;
            if(i == j) {
                if(k < a[i] + b[i] + 2) continue;
                ways = (k - a[i] - b[i]) * (k - a[i] - b[i] - 1);
            } else {
                if(k < a[i] + 1 || k < b[j] + 1) continue;
                ways = (k - a[i] - b[i]) * (k - a[j] - b[j]);
            }
            vt<int> ta(a), tb(b);
            ta[i]++;
            tb[j]++;
