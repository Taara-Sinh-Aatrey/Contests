...
int main() {
    ...
    map<pair<int, int>, Mint> ways;
    ways[{0, 0}] = 1;
    for (int i = 0; i < n; i++) {
        map<pair<int, int>, Mint> nways;
        int mn = min(a[i][0], a[i][1]), mx = max(a[i][0], a[i][1]);
        for (int x = mx - d; x <= mn + d; x++) {
            for (auto &[pr, way] : ways) {
                pair<int, int> npr = {pr.first + abs(x - mn), pr.second + abs(x - mx)};
                if (npr.first <= d && npr.second <= d) {
                    nways[npr] += ways[pr];
                }
            }
        }
