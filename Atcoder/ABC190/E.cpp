    ...
    for(int mask = 1; mask < (1 << k); mask++) {
    	for(int last = 0; last < k; last++) {
    		...
    		for(int i = 0; i < k; i++) {
	    		...
	    		amin(dp[mask | (1 << i)][i], dp[mask][last] + d[c[last]][c[i]]);
	    	}
    	}
    }
    int ans = *min_element(all(dp.back())) + 1;
    if(ans >= INF) ans = -1;
    cout << ans;
}
