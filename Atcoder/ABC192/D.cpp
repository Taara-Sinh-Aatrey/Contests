...
void solve() {
    ...
    cin >> M;
    if(X.length() == 1) {
        cout << (X.front() - '0' <= M ? 1 : 0);
        return;
    }
    // returns if number is small enough
    auto check = [&] (int base) -> bool {
        int num = 0;
        for(auto ch : X) {
            if(num > (M + base - 1) / base) {
                return 0;
            }
            num = num * base + (ch - '0');
        }
    ...
    int mn = *max_element(all(X)) - '0' + 1;
    int low = mn, high = 1e18;
    while(low <= high) {
        int mid = (low + high) / 2;
        if(check(mid)) {
            low = mid + 1;
