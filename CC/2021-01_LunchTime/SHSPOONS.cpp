...
void solve() {
    ...
    while(!pq.empty()) {
    	...
    	if(p.first <= cur) {
    		cur += p.first;
    		ans.pb({p.second, 1, p.first});
    	} else {
    		int extra = p.first - cur;
    		if(pq.empty()) {
    			cout << "-1\n";
    			return;
    		}
    		auto q = pq.top();;
    		pq.pop();
    		pq.emplace(q.first + extra, q.second);
    		ans.pb({p.second, q.second, extra});
    		ans.pb({p.second, 1, cur});
    		cur <<= 1;
    	}
    }
    assert(cur == sum);
