...
signed main() {
    ...
    while(t--) {
        ...
        vector<int> size(n);
        vector<Mint> ways(n, Mint(1));
        vector<long double> ans(n, 1.0);
        vector<pair<double, pair<int, Mint>>> pairs;
        ...
        function<void(int, int)> dfs1 = [&](int u, int prev) {
            ...
            for(int& nxt: g[u]) {
                ...
                dfs1(nxt, u);
                size[u] += size[nxt];
                ways[u] *= ways[nxt] / fact[size[nxt]];
            }
            ways[u] *= fact[size[u] - 1];
        }; 

        function<void(int, int)> dfs2 = [&](int u, int prev) {
            pairs.push_back({ans[u], {u + 1, ways[u]}});
            for(int& nxt: g[u]) {
                if(nxt == prev) {
                    continue;
                }
                ways[nxt] = ways[u] * size[nxt] / (n - size[nxt]);
                ans[nxt] = ans[u] * size[nxt] / (n - size[nxt]);
                dfs2(nxt, u);
            }
        }; 
        ...
        cout << pairs[k].second.first << " " << pairs[k].second.second << '\n';
