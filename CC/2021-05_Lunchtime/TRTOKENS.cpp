...
struct LeftistHeap{
    ...
    LeftistHeap<T> *l,*r;
    int s;
    LeftistHeap():l(nullptr),r(nullptr),s(1){}
};
...
using P=LeftistHeap<int>;
P buf[nmax];

void slv(){
    int n;sc.read(n);
    string str;sc.read(str);
    vvc<int> t(n);
    rng(i,1,n){
        int p;sc.read(p);p--;
        t[p].pb(i);
    ...
    ll ans=0;
    auto dfs=[&](auto self,int v,int d)->P*{
        P* res=NewNode(d,buf);
        for(auto to:t[v]){
            res=Merge(res,self(self,to,d+1));
        }
        if(str[v]=='1'){
            ans+=d;
        }else{
