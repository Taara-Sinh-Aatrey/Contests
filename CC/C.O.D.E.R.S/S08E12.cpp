...
void solve() {
    ...
    for (int i = 0; i <= m - n; i++) {
        {
            string s = t.substr(0, i) + S + t.substr(i + n);
            solve(s);
        }
        if (i > 0) {
            string s = t.substr(0, i - 1) + string(1, t[i - 1] ^ '0' ^ '1') + S + t.substr(i + n);
            solve(s);
        }
        if (i < m - n) {
            string s = t.substr(0, i) + S + string(1, t[i + n] ^ '0' ^ '1') + t.substr(i + n + 1);
            solve(s);
        }
        if (i > 0 && i < m - n) {
            string s = t.substr(0, i - 1) + string(1, t[i - 1] ^ '0' ^ '1') + S + string(1, t[i + n] ^ '0' ^ '1') + t.substr(i + n + 1);
            solve(s);
        }
