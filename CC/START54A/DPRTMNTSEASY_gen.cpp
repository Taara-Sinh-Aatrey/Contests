...
void solve() {
    ...
    for (int i = 0, j; i < n; i = j + 1) {
        ...
        groups.emplace_back();
        for (int x = i; x <= j; x++) {
            groups.back().emplace_back(nodes[x]);
            for (int y = x + 1; y <= j; y++) {
                g.emplace_back(nodes[x], nodes[y]);
            }
        }
        leaders.emplace_back(nodes[rand(i, j)]);
    }
    for (int x = 0; x < leaders.size(); x++) {
        for (int y = x + 1; y < leaders.size(); y++) {
            g.emplace_back(leaders[x], leaders[y]);
        }
    }
    print(n, g.size());
    for (auto &p : g) {
    ...
    print(groups.size());
    for (auto &group : groups) print(group);
    print(leaders);
}
