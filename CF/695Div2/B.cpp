...
void solve(){
	...
	// Rep(i, 1, n - 2){
	// 	amax(sub, hill[i - 1] + vall[i] + hill[i + 1]);
	// 	amax(sub, vall[i - 1] + hill[i] + vall[i + 1]);
	// }
	auto fun = [&](ll idx, ll val){
		ll cur = ans;
		ll num = a[idx];
		a[idx] = val;
		for(int i = max(1ll, idx - 1); i <= min(n - 2, idx + 1); ++i){
			cur -= hill[i] + vall[i];
			if(a[i] < a[i + 1] && a[i] < a[i - 1]){
				cur++;
			}
