...
void solve() {
    ...
    for(int i = 0, j = 0; i < n && j < m; i++) {
        if(s[i] == t[j]) {
            if(!taken.empty())
                amax(ans, i - taken.back());
            taken.pb(i);
            j++;
        }
    }
    Rev(i, m) {
        int nxt = (i == m - 1 ? n : taken[i + 1]);
        int ch = s[taken[i]] - 'a';
        auto it = lower_bound(all(pos[ch]), nxt);
        // dbg(ch, pos[ch]);
        // dbg(ch, it);
        assert(it != pos[ch].begin());
        it--;
        taken[i] = *it;
        if(i > 0)
            amax(ans, taken[i] - taken[i - 1]);
    }
    cout << ans << '\n';
}
