...
void solve() {
    ...
    for(int i = 0; i < n; i++) {
        ...
        cur &= sum;
        if(cur.any()) {
            int s = cur._Find_first();

            int x = -1, y = -1, z, w = b[i].second;

            for(int j = i - 1; j >= 0; j--) {
                if(b[i].first + b[j].first == s) {
                    z = b[j].second;
                    cout << "YES\n";
                    cout << x << " " << y << " " << z << " " << w;
                    return;
                ...
                if(y == -1) {
                    x = lower_bound(all(b), make_pair(s - b[j].first, -INF)) - b.begin();
                    if(b[x].first + b[j].first == s) {
                        x = b[x].second;
                        y = b[j].second;
                    }
                }
