...
template<typename T>
typename set<T>::iterator get_next(set<T>& st, T se) {
    auto it = st.upper_bound(se);
    if(it == st.end()) {
        it = st.begin();
    }
    return it;
...
void solve() {
    ...
    while(!st.empty() && !bad.empty()) {
        auto it = get_next(bad, {last, -1});
        auto [x, y] = *it;
        ans.pb(y + 1);
        bad.erase(it);
        int nxt = *get_next(st, y);
        it = bad.find({y, nxt});
        if(it != bad.end()) {
            bad.erase(it);
        }
        st.erase(y);
        if(__gcd(a[x], a[nxt]) == 1) {
            bad.insert({x, nxt});
        }
        last = y;
    }
