...
void solve() {
    ...
    for(int i = 1; i <= n; i++) {
        ...
        set<int> new_visited;
        while(sz(visited) + sz(new_visited) <= m && it != visited.rend()) {
            int val = *it;
            // dbg(i, val);
            for(int iter = 1; iter <= y; iter++) {
                ...
                int rm = val % MAX;
                val = val + (MAX - (rm == 0 ? MAX : rm));
                val /= MAX;
                // dbg(val);
                if(val > m || visited.count(val)) {
                    break;
                }
                ans[val] = i;
                new_visited.insert(val);
            }
            it++;
        }
        for(auto& x: new_visited) {
            visited.insert(x);
        }
    }
