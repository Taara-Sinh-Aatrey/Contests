...
void solve() {
    ...
    scan(a);
    vector<vector<int>> ord(2, vector<int>(n));
    queue<int> qu;
    string s(n, '0');
    for (int rep = 0; rep < 2; rep++) {
        iota(ord[rep].begin(), ord[rep].end(), 0);
        sort(ord[rep].begin(), ord[rep].end(), [&] (auto &i, auto &j) {
            return a[rep][i] < a[rep][j];
        });
        qu.emplace(ord[rep].back());
        ord[rep].pop_back();
    }
    while (!qu.empty()) {
        int i = qu.front();
