...
void solve() {
    ...
    int na = a.size(), nb = b.size(), n = s.size();
    
    int lcs = -1;
    for(int i = 1; i < n; i++) {
        // dbg(i, SA.suffix[i - 1], SA.suffix[i]);
        if((SA.suffix[i] < na) ^ (SA.suffix[i - 1] < na)) {
            if(lcs == -1 || SA.lcp[i] > SA.lcp[lcs]) {
                lcs = i;
            }
        }
    }
    
    cout << s.substr(SA.suffix[lcs], SA.lcp[lcs]);
}
