...
void solve() {
    ...
    // dp[i][j] : min time to shift starting i+1 elements with value 1 from (0 to j)

    for(int i = 0; i < m; i++) {
        for(int j = 0; j < n; j++) {
            dp[i][j] = j > 0 ? dp[i][j - 1] : INF;
            if(a[j] == 1) continue;
            if(i == 0) {
                amin(dp[i][j], abs(at[i] - j));
            }
            else if(j != 0) {
                amin(dp[i][j], dp[i - 1][j - 1] + abs(at[i] - j));
            } 
        }
    }
