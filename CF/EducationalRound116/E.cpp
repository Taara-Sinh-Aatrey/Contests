...
void solve() {
    int n, x;
    scan(n, x);
    vector<vector<Mint>> p(x + 1, vector<Mint>(n + 1, 0));
    for (int i = 0; i <= x; i++) {
        p[i][0] = 1;
        for (int j = 1; j <= n; j++)
            p[i][j] = p[i][j - 1] * i;
    ...
    for (int alive = 2; alive <= n; alive++)
        for (int lim = 1; lim <= x; lim++)
            for (int killed = 0; killed <= alive; killed++)
                dp[alive][lim] += C(alive, killed) * p[min(alive - 1, lim)][killed] * dp[alive - killed][max<int>(0, lim - alive + 1)];
    print(dp[n][x]);
}
