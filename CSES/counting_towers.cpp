...
vt<int> blank(N, 0), complete(N, 0), vertical(N, 0);
...
signed main()
{
    ...
    for(int i = 1; i < N; i++) {
        complete[i] = 2 * complete[i - 1] + vertical[i - 1] + blank[i - 1];
        complete[i] %= mod;
        vertical[i] = 3 * (vertical[i - 1] + complete[i - 1]);
        vertical[i] %= mod;
        blank[i] = blank[i - 1] + complete[i - 1];
        blank[i] %= mod;
    }
