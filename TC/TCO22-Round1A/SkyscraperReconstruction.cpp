...
class SkyscraperReconstruction {
	public:
	vector <int32_t> reconstruct(string s) {
	    int n = s.size();
...
namespace moj_harness {
	...
	int run_test_case(int casenum__) {
		...
		case 0: {
			...
			int expected__[]          = {3, 1, 5, 4, 2, 7, 6 };

			std::clock_t start__      = std::clock();
			vector <int> received__   = SkyscraperReconstruction().reconstruct(visibility);
			return verify_case(casenum__, vector <int>(expected__, expected__ + (sizeof expected__ / sizeof expected__[0])), received__, clock()-start__);
		}
		case 1: {
			string visibility         = "OXXXX";
			int expected__[]          = {5, 2, 4, 3, 1 };
		...
		case 3: {
			string visibility         = "OOOXOXX";
			int expected__[]          = {1, 3, 5, 4, 7, 2, 6 };
