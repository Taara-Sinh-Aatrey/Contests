...
public:
    ...
    inline void push(int x, int l, int r) {
        ...
        if (tree[x].add != 0) {
            // assert(tree[x].mul == 1);
            if(tree[x + 1].mul != 0 && l != y) {
                push(x + 1, l, y);
            }
            if(tree[z].mul != 0 && y + 1 != r) {
                push(z, y + 1, r);
            }
            tree[x + 1].apply(l, y, {1, tree[x].add});
            tree[z].apply(y + 1, r, {1, tree[x].add});
            tree[x].add = 0;
        }
        if (tree[x].mul != 0) {
            if(tree[x + 1].add != 0 && l != y) {
                push(x + 1, l, y);
            }
            if(tree[z].add != 0 && y + 1 != r) {
                push(z, y + 1, r);
            }
            tree[x + 1].apply(l, y, {2, tree[x].mul});
            tree[z].apply(y + 1, r, {2, tree[x].mul});
            tree[x].mul = 0;
        }
    }
