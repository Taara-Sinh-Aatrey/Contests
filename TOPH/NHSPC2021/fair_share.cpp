...
void solve() {
    ...
    for(int i = 0; i < k && l <= r; i++) {
        ...
        int mid = offset + sum / 2;
        
        auto it = lower_bound(a.begin() + l, a.begin() + r + 1, mid) - a.begin();
        
        pair<int, pair<int, int>> take{a[it] - a[l - 1], {l, it}};
        if(a[r] - a[it] > take.first) {
            take = {a[r] - a[it], {it + 1, r}};
        }
        
        if(it > l) {
            it--;
            pair<int, pair<int, int>> take2{a[it] - a[l - 1], {l, it}};
            if(a[r] - a[it] > take2.first) {
                take2 = {a[r] - a[it], {it + 1, r}};
            }
            
            amin(take, take2);
        }
        
        auto[nl, nr] = take.second;
        l = nl;
        r = nr;
    }
    
    
    cout << (r >= l ? a[r] - a[l - 1] : 0) << '\n';
}
