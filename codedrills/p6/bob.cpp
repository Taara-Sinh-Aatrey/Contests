...
void solve() {
    ...
    int xx, yy;
    cin >> xx >> yy;
    Mint x = Mint(xx), y = Mint(yy);
    Mint pp = x/y;
    // E[i][j] = c[i][j]*x + v[i][j]
    // x = E[0][0];
    vt<vt<Mint>> v(n, vt<Mint>(m, Mint(0)));
    vt<vt<Mint>> c(n, vt<Mint>(m, Mint(0)));
    int f = 0;
    Mint ans = 0;
    for (int i = n - 1; i >= 0; --i) {
        for (int j = m - 1; j >= 0; --j) {
            ...
            if(a[i][j] == '.' || a[i][j] == '1'){
                // if(i + 1 < n) {
                //     c[i][j] = c[i + 1][j] * (Mint(1) - pp);
                //     v[i][j] = v[i + 1][j] * (Mint(1) - pp);
                // }
                // if(j + 1 < m) {
                //     c[i][j] = c[i][j + 1] * pp;
                //     v[i][j] = v[i][j + 1] * pp;
