...
void solve() {
    ...
    string s;
    scan(n, s);
    Mint ans = 0, cur = 0, l1 = -1, l2 = -1, len = 0, c1 = 0, c2 = 0;
    int p = -1, init_p = -1;
    Mint minus_one = Mint(-1);
    vector<Mint> last(2, minus_one);
    for(int i = 0; i < n; i++) {
        char ch = s[i];
        int np = p;
        if(ch == '.') {
            ans = ans * 2 + (cur + c2) * len;
            cur = cur * 2 + c1 * len;
            c2 = c2 * 2 + c1 * len;
            c1 *= 2;
            if(l1 != minus_one && p != init_p && last[1 - init_p] != minus_one) {
                ans += (len - l1) * (last[1 - init_p] + 1);
                cur += last[1 - init_p] + 1;
                c2 += len - l1;
                c1++;
            }
            for(int _p : {0, 1}) {
                if(last[_p] != minus_one) last[_p] += len;
            }
            len *= 2;
            continue;
        }
