...
#define int int64_t
    ...
    for (int caseno = 1; caseno <= testcases; caseno++) {
        ...
        SEGTREE_HASH HASH(a);
        const int Q_MAX = 1e6;
        vector<oset<int>> occ(M + Q_MAX);
        vector<vector<int>> occ(M);
        for (int i = 1; i <= n; i++) {
            occ[a[i]].emplace(i);
        }
        auto get_occurence = [&] (int e, int l, int r) {
            // int cnt = 0;
            // for (int i = l; i <= r; i++) {
            //     cnt += a[i] == e;
            // }
            ...
            if (occ[e].empty())
                return int(0);
            int x = occ[e].order_of_key(r + 1) - occ[e].order_of_key(l);
            return x;
        };
        auto fun = [&] (int l, int r) {
            ...
            if (e >= id.size())
                return 0;
            int tot = get_occurence(e, l, r);
            int half = get_occurence(e, l, m - 1);
            dbg(tot, half);
            if (tot % 2 != 1) {
                return 0;
            }
            ...
            if (half == (tot + 1) / 2) {
                // l, m
                Mint left = HASH.get(l, m).sum;
                // dbg(left);
                left -= e * primes[e];
                // m + 1, r
                Mint right = HASH.get(m + 1, r).sum;
                // dbg(e, tot, left, right);
                if (left == right)
                    return 1;
            }
