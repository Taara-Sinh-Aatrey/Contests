...
void solve() {
    ...
    for(int i = 1; i < n; i++) {
        ...
        u--, v--;
        amax(MAX, load);
        g[u].push_back({v, load, toll});
        g[v].push_back({u, load, toll});
    }

    segtree st(MAX + 1);
    vt<vt<array<int, 2>>> Q(n);
    vt<int> ans(q);
    for(int i = 0; i < q; i++) {
        int u, load;
        cin >> u >> load;
        u--;
        Q[u].push_back({load, i});
    }

    function<void(int, int, int, int)> dfs = [&](int u, int prev, int pload, int ptoll) -> void {
        st.modify(pload, pload, ptoll);

        for(auto [qload, idx]: Q[u]) {
            ans[idx] = st.get(1, min(qload, MAX)).g;
        }

        for(auto [v, load, toll]: g[u]) {
            if(v == prev) continue;
            dfs(v, u, load, toll);
        }
        st.modify(pload, pload, 0);
    };
