...
void solve() {
    ...
    // vector<vector<Mint>> prob(lvl, vector<Mint>(n));
    vector<vector<Mint>> a(lvl, vector<Mint>(n));
    vector<vector<Mint>> b(lvl, vector<Mint>(n));
    const Mint den = 1000000; 
    vector<Mint> rootProb(n);
    rootProb[0] = k;
    rootProb[0] /= den;
    // vector<Mint> a(n), b(n);
    for (int i = 1; i < n; i++) {
    ...
    function<void(int)> dfs = [&](int u) {
        depth[u] = p[0][u] == -1 ? 0 : depth[p[0][u]] + 1;
        for(auto &v : g[u]) {
            rootProb[v] = rootProb[u] * a[0][v] + (1 - rootProb[u]) * b[0][v];
            dfs(v);
        }    
    };
    dfs(0);
    for(int i = 1; i < lvl; i++) {
        for(int node = 0; node < n; node++){
            ...
            if(x != -1) {
                p[i][node] = p[i - 1][x];
                a[i][node] = a[i - 1][node] * a[i - 1][x] + (1 - a[i - 1][node]) * b[i - 1][x];
                b[i][node] = b[i - 1][node] * a[i - 1][x] + (1 - b[i - 1][node]) * b[i - 1][x];
            }
        }
    }
