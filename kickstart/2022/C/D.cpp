...
void solve() {
    ...
    auto update = [&] (vector<Mint> &vec, int i, int j, int coeff) {
        for (int l = 1; l <= j - i + 1; l++) {
            vec[l] += coeff * dp[i][j][l];
        }
    };
    for (int i = n - 1; i >= 0; i--) {
        for (int j = i; j < n; j++) {
            dp[i][j].resize(j - i + 2, 0);
            if (i == j) {
                dp[i][j][1] = 1;
                continue;
            }
            update(dp[i][j], i, j - 1, 1);
            update(dp[i][j], i + 1, j, 1);
            if (s[i] == s[j]) {
                dp[i][j][2] += 1;
            }
            if (i + 1 <= j - 1) {
                update(dp[i][j], i + 1, j - 1, -1);
                if (s[i] == s[j]) {
                    int x = i + 1, y = j - 1;
